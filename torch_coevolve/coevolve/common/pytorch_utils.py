...
from coevolve.common.consts import DEVICE, NONLINEARITIES
...
class IndexSelect(Function):
    @staticmethod
    def forward(ctx, holder, indices, dummy):
        ctx.holder = holder
        ctx.indices = indices
        return holder.weight[indices]

    @staticmethod
    def backward(ctx, grad_output):
        holder = ctx.holder
        indices = ctx.indices
        holder.weight.grad[indices] += grad_output
        return None, None, None


class SpEmbedding(nn.Module):
    def __init__(self, num_embeddings, embed_dim):
        super(SpEmbedding, self).__init__()
        self.num_embeddings = num_embeddings
        self.embed_dim = embed_dim
        v = torch.zeros(num_embeddings, embed_dim, dtype=torch.float32)
        glorot_uniform(v)
        self.weight = Parameter(v)
        self.weight.grad = torch.zeros(self.num_embeddings, self.embed_dim, dtype=torch.float32)

        self.dummy = Parameter(torch.zeros(1, 1, dtype=torch.float32))
    ...
    def normalize(self, max_norm):
        ...
        with torch.set_grad_enabled(False):
            norms = torch.sum(self.weight ** 2, dim=1) / max_norm
            norms[norms < 1] = 1
            tmp = self.weight / norms.view(-1, 1)
            self.weight.data = tmp.data
 
    def forward(self, indices):        
        return IndexSelect.apply(self, indices, self.dummy)
